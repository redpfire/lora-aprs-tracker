...
char *aprs_beacon_tick(struct GNSS_Data *gnss_data, struct APRS_Beacon_State *state,
    ...
    if (gnss_data->latitude == state->last_gnss_data.latitude &&
        gnss_data->longitude == state->last_gnss_data.longitude && state->was_first_fix == 0) {
        return NULL;
    }

    const int bearing_delta = abs(state->last_gnss_data.bearing - gnss_data->bearing);
    // TOFIX: bool maybe? Not in C, i guess.
    size_t is_corner_peg =
        aprs_sb_check_corner_peg(aprs_sb_config, gnss_data, bearing_delta, state->ms_since_last_beacon);
