...
void lora_thread(void *dummy1, void *dummy2, void *dummy3) {
    ...
    for(;;) {
        ...
        // Ideally there should be a struct sent over the fifo with void* to data and its size
...
void spawn_lora_thread() {
    k_thread_create(&lora_thread_data, lora_thread_stack_area,
            K_THREAD_STACK_SIZEOF(lora_thread_stack_area),
